...
--
-- Additive only: two new tables, four nullable columns on `routes` recording a merge
-- decision, their indexes and foreign keys. There is no DROP, no ALTER of an existing
-- column's type or nullability, and nothing touching a revision table.
--
...
-- CreateIndex
CREATE UNIQUE INDEX "duplicate_flags_routeId_duplicateOfId_reporterId_key" ON "duplicate_flags"("routeId", "duplicateOfId", "reporterId");
...
-- AddForeignKey
ALTER TABLE "duplicate_flags" ADD CONSTRAINT "duplicate_flags_reporterId_fkey" FOREIGN KEY ("reporterId") REFERENCES "users"("id") ON DELETE SET NULL ON UPDATE CASCADE;
