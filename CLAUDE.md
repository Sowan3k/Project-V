...
**This repository is public (since 2026-09-02).** Every file and every commit is readable
by anyone, including commits later changed — so redacting a file does not un-publish what
it used to contain. Never write a connection string, endpoint hostname, password, API key,
token or personal address into a tracked file, including docs, fixtures and verification
logs. Record the shape of a finding, not the value: "host ends in `-pooler`", never the
host. See `Test.md` §10 for the history scan and the rotation rule.
...
**The deployed application needs `DATABASE_URL` and nothing else.** `directUrl` is read
only by the Prisma CLI (migrate, introspect), which is verified — a runtime query succeeds
with `DATABASE_URL_UNPOOLED` absent. So the direct, unpooled credential stays on
workstations and never goes into Vercel, and migrations are always applied deliberately by
a person following the branch discipline below rather than automatically on deploy. A
deploy step that ran migrations would be able to destroy revision history without review,
